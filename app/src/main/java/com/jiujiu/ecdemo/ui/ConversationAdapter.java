...
public class ConversationAdapter extends CCPListAdapter <Conversation> {
    ...
    public ConversationAdapter(Context ctx , OnListAdapterCallBackListener listener) {
        ...
        colorStateLists = new ColorStateList[]{
                ResourceHelper.getColorStateList(ctx, com.jiujiu.ecdemo.R.color.normal_text_color),
                ResourceHelper.getColorStateList(ctx, com.jiujiu.ecdemo.R.color.ccp_list_textcolor_three)
        };
    }
    ...
    protected final CharSequence getConversationTime(Conversation conversation) {
        if(conversation.getSendStatus() == ECMessage.MessageStatus.SENDING.ordinal()) {
            return mContext.getString(com.jiujiu.ecdemo.R.string.conv_msg_sending);
        }
    ...
    protected final CharSequence getConversationSnippet(Conversation conversation) {
        ...
        if (conversation.getMsgType() == ECMessage.Type.VOICE.ordinal()) {
            return fromNickName + mContext.getString(com.jiujiu.ecdemo.R.string.app_voice);
        } else if(conversation.getMsgType() == ECMessage.Type.FILE.ordinal()) {
            return fromNickName + mContext.getString(com.jiujiu.ecdemo.R.string.app_file);
        } else if(conversation.getMsgType() == ECMessage.Type.IMAGE.ordinal()) {
            return fromNickName + mContext.getString(com.jiujiu.ecdemo.R.string.app_pic);
        } else if(conversation.getMsgType() == ECMessage.Type.VIDEO.ordinal()) {
            return fromNickName + mContext.getString(com.jiujiu.ecdemo.R.string.app_video);
        }else if(conversation.getMsgType()== ECMessage.Type.LOCATION.ordinal()){
            return fromNickName + mContext.getString(com.jiujiu.ecdemo.R.string.app_location);

        }
        String snippet = fromNickName + conversation.getContent();
        if(conversation.getSessionId().equals(isAtSession)) {
            return Html.fromHtml(mContext.getString(com.jiujiu.ecdemo.R.string.conversation_at, snippet));
        }
        return snippet;
    }
    ...
    public static Drawable getChattingSnippentCompoundDrawables(Context context , Conversation conversation) {
        if(conversation.getSendStatus() == ECMessage.MessageStatus.FAILED.ordinal()) {
            return DemoUtils.getDrawables(context, com.jiujiu.ecdemo.R.drawable.msg_state_failed);
        } else if (conversation.getSendStatus() == ECMessage.MessageStatus.SENDING.ordinal()) {
            return DemoUtils.getDrawables(context, com.jiujiu.ecdemo.R.drawable.msg_state_sending);
        } else {
            return null;
        }
    }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if(convertView == null || convertView.getTag() == null) {
            view = View.inflate(mContext , com.jiujiu.ecdemo.R.layout.conversation_item, null);

            mViewHolder = new ViewHolder();
            mViewHolder.user_avatar = (ImageView) view.findViewById(com.jiujiu.ecdemo.R.id.avatar_iv);
            mViewHolder.prospect_iv = (ImageView) view.findViewById(com.jiujiu.ecdemo.R.id.avatar_prospect_iv);
            mViewHolder.nickname_tv = (EmojiconTextView) view.findViewById(com.jiujiu.ecdemo.R.id.nickname_tv);
            mViewHolder.tipcnt_tv = (TextView) view.findViewById(com.jiujiu.ecdemo.R.id.tipcnt_tv);
            mViewHolder.update_time_tv = (TextView) view.findViewById(com.jiujiu.ecdemo.R.id.update_time_tv);
            mViewHolder.last_msg_tv = (EmojiconTextView) view.findViewById(com.jiujiu.ecdemo.R.id.last_msg_tv);
            mViewHolder.image_input_text = (ImageView) view.findViewById(com.jiujiu.ecdemo.R.id.image_input_text);
            mViewHolder.image_mute = (ImageView) view.findViewById(com.jiujiu.ecdemo.R.id.image_mute);
        ...
        if(conversation != null) {
            ...
            if(conversation.getSessionId().toUpperCase().startsWith("G")) {
                ...
                if(bitmap != null) {
                    ...
                    mViewHolder.user_avatar.setImageResource(com.jiujiu.ecdemo.R.drawable.group_head);
                ...
                if(conversation.getMsgType() == 1000) {
                    mViewHolder.user_avatar.setImageResource(com.jiujiu.ecdemo.R.mipmap.ic_launcher);
                } else {
        ...
        if(isTop&&!conversation.getSessionId().equals(GroupNoticeSqlManager.CONTACT_ID)){
            view.setBackgroundColor(mContext.getResources().getColor(com.jiujiu.ecdemo.R.color.list_bg_gaoliang));
        }else {
            view.setBackgroundDrawable(mContext.getResources().getDrawable(com.jiujiu.ecdemo.R.drawable.list_item_selector));
