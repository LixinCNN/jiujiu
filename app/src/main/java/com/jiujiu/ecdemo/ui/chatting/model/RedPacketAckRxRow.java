...
public class RedPacketAckRxRow extends BaseChattingRow {
    ...
    public View buildChatView(LayoutInflater inflater, View convertView) {
        ...
        if (convertView == null) {
            convertView = new ChattingItemContainer(inflater, com.jiujiu.ecdemo.R.layout.chatting_item_redpacket_ack_from);
            ...
            if (message.getType() == ECMessage.Type.TXT) {
                ...
                if (jsonObject != null) {
                    ...
                    if (currentUserId.equals(recieveUserId) && currentUserId.equals(sendUserId)) {
                        text = context.getResources().getString(com.jiujiu.ecdemo.R.string.money_msg_take_money);
                        holder.getRedPacketAckMsgTv().setVisibility(View.VISIBLE);
                    } else if (currentUserId.equals(sendUserId)) {
                        //我仅仅是发送者
                        text = String.format(context.getResources().getString(com.jiujiu.ecdemo.R.string.money_msg_someone_take_money), recieveUserNick);
                        holder.getRedPacketAckMsgTv().setVisibility(View.VISIBLE);
                    } else if (currentUserId.equals(recieveUserId)) {
                        //我仅仅是接收者
                        text = String.format(context.getResources().getString(com.jiujiu.ecdemo.R.string.money_msg_take_someone_money), sendUserNick);
