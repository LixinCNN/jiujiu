...
public class InterPhoneItem  {
    ...
    private void initView(View view) {
        mInterSpeak = (ImageView) view.findViewById(com.jiujiu.ecdemo.R.id.inter_phone_join_icon);
        mUsernameView = (TextView) view.findViewById(com.jiujiu.ecdemo.R.id.name);
        mUserActionView = (TextView) view.findViewById(com.jiujiu.ecdemo.R.id.action_tips);
    }
    ...
    public void setInterMember(ECInterPhoneMeetingMember member) {
        ...
        if(member.getMember().equals(CCPAppManager.getClientUser().getUserId())) {
            mInterSpeak.setImageResource(com.jiujiu.ecdemo.R.drawable.inter_person_icon);
        }
    }
    ...
    private boolean setInterMemberOnline(ECInterPhoneMeetingMember.Online online) {
        if(online != ECInterPhoneMeetingMember.Online.ONLINE) {
            // 实时对讲成员状态显示未加入
            mInterSpeak.setImageResource(com.jiujiu.ecdemo.R.drawable.status_wait);
            mUserActionView.setText(com.jiujiu.ecdemo.R.string.str_join_wait);
            return false;
        }
        // 实时对讲成员在线
        return true;
    ...
    private void setInterMemberMic(ECInterPhoneMeetingMember.Mic mic) {
        if(mic == ECInterPhoneMeetingMember.Mic.MIC_CONTROLLER) {
            // 实时对讲成员控麦
            mInterSpeak.setImageResource(com.jiujiu.ecdemo.R.drawable.status_speaking);
            mUserActionView.setText(com.jiujiu.ecdemo.R.string.str_join_speaking);
            return ;
        }
        mInterSpeak.setImageResource(com.jiujiu.ecdemo.R.drawable.status_join);
        mUserActionView.setText(com.jiujiu.ecdemo.R.string.str_join_success);
    }
}
