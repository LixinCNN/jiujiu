...
public class ApplyWithGroupPermissionActivity extends ECSuperActivity implements
	...
	protected int getLayoutId() {
		return com.jiujiu.ecdemo.R.layout.apply_group_activity;
	}
	...
	private void initView() {
		mGroupProfileView = (GroupProfileView) findViewById(com.jiujiu.ecdemo.R.id.group_file);
		mNotice = (EditText) findViewById(com.jiujiu.ecdemo.R.id.group_notice);
		mNotice.setEnabled(false);
		
		TextView view = (TextView) findViewById(com.jiujiu.ecdemo.R.id.red_btn);
		view.setBackgroundResource(com.jiujiu.ecdemo.R.drawable.btn_style_red);
		view.setOnClickListener(this);
		view.setText(com.jiujiu.ecdemo.R.string.group_apply_btn);
	}
	...
	private void syncGroupInf(String groupId) {
		...
		if(!TextUtils.isEmpty(mGroup.getName())) {
			...
			getTopBarView().setTopBarToStatus(1, com.jiujiu.ecdemo.R.drawable.topbar_back_bt, -1, mGroup.getName(), this);
		}
    ...
    private void applyGroup(String declare) {
        mPostingdialog = new ECProgressDialog(this, com.jiujiu.ecdemo.R.string.loading_press);
        mPostingdialog.show();
        if(TextUtils.isEmpty(declare)) {
            String userName = CCPAppManager.getClientUser().getUserName();
            declare = getString(com.jiujiu.ecdemo.R.string.group_apply_reason, userName);
        }
