...
public class +CCPAlertDialog {
	...
	public CCPAlertDialog(Context context , int title, int[] normal , int special , int cancle){
		...
		this.mDialog = new Dialog(context, com.jiujiu.ecdemo.R.style.CCPAlertdialog);
		this.mTitle = title;
	...
	public Dialog create(){
		...
		final LinearLayout mLlayout = (LinearLayout) this.mInflater.inflate(com.jiujiu.ecdemo.R.layout.ccp_dialog_menu_layout, null);
		mLlayout.setMinimumWidth(LayoutParams.MATCH_PARENT);
		mListView = (ListView)mLlayout.findViewById(com.jiujiu.ecdemo.R.id.content_list);
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (this.mList != null) {
				...
				if (btnType != TYPE_BUTTON_NORMAL) {
					...
					if (btnType == TYPE_BUTTON_SPECIAL) {
						view = mInflater.inflate(
								com.jiujiu.ecdemo.R.layout.ccp_dialog_menu_list_layout_special,
								null);
					} else if (btnType == TYPE_BUTTON_CANCLE) {
						view = mInflater
								.inflate(
										com.jiujiu.ecdemo.R.layout.ccp_dialog_menu_list_layout_cancel,
										null);
					} else if (btnType == TYPE_BUTTON_TITLE){
						view = mInflater
						.inflate(
								com.jiujiu.ecdemo.R.layout.ccp_dialog_menu_list_layout_title,
								null);
					}
					((TextView) view.findViewById(com.jiujiu.ecdemo.R.id.popup_text)).setText(btnStr);
					return view;
				} 
				...
				if (convertView != null && (PopuHolder)convertView.getTag() != null &&((PopuHolder)convertView.getTag()).type == TYPE_BUTTON_NORMAL) {
					...
					view = mInflater
							.inflate(com.jiujiu.ecdemo.R.layout.ccp_dialog_menu_list_layout, null);
					holder = new PopuHolder();
					holder.title = (TextView) view.findViewById(com.jiujiu.ecdemo.R.id.popup_text);
