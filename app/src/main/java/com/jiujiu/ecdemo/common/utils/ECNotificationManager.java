...
import com.jiujiu.ecdemo.ui.voip.ECVoIPBaseActivity;
...
public class ECNotificationManager {
    ...
    public final void showCustomNewMessageNotification(Context context , String pushContent ,String fromUserName ,String sessionId ,  int lastMsgType) {
        ...
        Notification notification = NotificationUtil.buildNotification(context, com.jiujiu.ecdemo.R.drawable.title_bar_logo, defaults, shake, tickerText, contentTitle, contentText, null, pendingIntent);
    ...
    public final String getTickerText(Context contex ,String fromUserName ,int msgType) {
        if(msgType == ECMessage.Type.TXT.ordinal()) {
            return contex.getResources().getString(com.jiujiu.ecdemo.R.string.notification_fmt_one_txttype, fromUserName);
        } else if (msgType == ECMessage.Type.IMAGE.ordinal()) {
            return contex.getResources().getString(com.jiujiu.ecdemo.R.string.notification_fmt_one_imgtype, fromUserName);
        } else if (msgType == ECMessage.Type.VOICE.ordinal()) {
            return contex.getResources().getString(com.jiujiu.ecdemo.R.string.notification_fmt_one_voicetype, fromUserName);
        } else if (msgType == ECMessage.Type.FILE.ordinal()) {
            return contex.getResources().getString(com.jiujiu.ecdemo.R.string.notification_fmt_one_filetype, fromUserName);
        } else if (msgType == GroupNoticeSqlManager.NOTICE_MSG_TYPE) {
            return contex.getResources().getString(com.jiujiu.ecdemo.R.string.str_system_message_group_notice);
        } else {
    ...
    public final String getContentTitle(Context context ,int sessionUnreadCount, String fromUserName) {
        if(sessionUnreadCount > 1) {
            return context.getString(com.jiujiu.ecdemo.R.string.app_name);
        }
    ...
    public final String getContentText(Context context , int sessionCount , int sessionUnread , String pushContent ,int lastMsgType) {
        ...
        if (sessionCount > 1) {
            ...
            return context.getResources().getQuantityString(
                    com.jiujiu.ecdemo.R.plurals.notification_fmt_multi_msg_and_talker,1,
                    sessionCount, sessionUnread);
        }

        if(sessionUnread > 1) {
            return context.getResources().getQuantityString(
                    com.jiujiu.ecdemo.R.plurals.notification_fmt_multi_msg_and_one_talker, sessionUnread,sessionUnread);
        }

        if(lastMsgType == ECMessage.Type.TXT.ordinal()) {
            return pushContent;
        } else if (lastMsgType == ECMessage.Type.FILE.ordinal()) {
            return context.getResources().getString(com.jiujiu.ecdemo.R.string.app_file);
        } else if (lastMsgType == ECMessage.Type.VOICE.ordinal()) {
            return context.getResources().getString(com.jiujiu.ecdemo.R.string.app_voice);
        } else if (lastMsgType == ECMessage.Type.IMAGE.ordinal()) {
            return context.getResources().getString(com.jiujiu.ecdemo.R.string.app_pic);
        } else {
            return pushContent;
        }
    ...
    public final void showKickoffNotification(Context context , String kickofftext) {
        ...
        intent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
        PendingIntent pendingIntent = PendingIntent.getActivity(mContext, 35, intent, PendingIntent.FLAG_UPDATE_CURRENT);
        Notification notification = new Notification(com.jiujiu.ecdemo.R.drawable.ic_launcher, null, System.currentTimeMillis());
    ...
    public static void showCallingNotification(ECVoIPCallManager.CallType callType) {
        try {
            ...
            String topic = getInstance().mContext.getString(com.jiujiu.ecdemo.R.string.ec_voip_is_talking_tip);
            Notification notification = new Notification(com.jiujiu.ecdemo.R.drawable.title_bar_logo, null,
            ...
            PendingIntent contentIntent = PendingIntent.getActivity(getInstance().mContext,
                    com.jiujiu.ecdemo.R.string.app_name,
                    intent,
