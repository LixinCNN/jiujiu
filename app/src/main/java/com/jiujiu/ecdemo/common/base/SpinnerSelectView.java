...
public class SpinnerSelectView extends Button implements View.OnClickListener{
    ...
    private void initTypedArray(Context context, AttributeSet attrs, int defStyle) {
        TypedArray typedArray = context.obtainStyledAttributes(attrs, com.jiujiu.ecdemo.R.styleable.SpinnerView, defStyle, 0);
        int items = typedArray.getResourceId(com.jiujiu.ecdemo.R.styleable.SpinnerView_Spinner_item ,-1);
        mDropTitle = typedArray.getResourceId(com.jiujiu.ecdemo.R.styleable.SpinnerView_Spinner_DropDownTitle ,-1);
        typedArray.recycle();
        ...
        if(mDropTitle <= 0) {
            mDropTitle = com.jiujiu.ecdemo.R.string.app_tip;
        }
    }
